...
public class A_array2 {
	public static void main(String[] args) {
		...
		System.out.println(dArr.hachCode());
		*/
	...
	arr = new int[7];
	System.out.println(arr.hashCode());
	//주소값이 변경되었다 = > 새로운 곳을 참조하고있다
	
	/*연결이 끊어진 기존 배열은 Heap영역에 둥둥 떠다닌다(어떤변수에도 참조되지않는 상태, 불러올수 없음)
	 * => 일정시간이 지나면  "가비지 컬렉터(GC)"가 알아서 지워줌
	 * => 자바에서의 "자동 메모리 관리 " 특징 => 개발자가 코드에만 집중할 수 있다.
	 */
	
	arr = null;
	//배열을 강제로 삭제시키고자 한다면 => 연결고리를 끊어주면 된다 .(null을 대입)
	
	int[] arr2 = {1, 1, 1, 1, 1};
	int[] arr3 = {1, 1, 1, 1, 1};
	System.out.println(arr2 == arr3);// false =>각 참조변수는 == 을 통해서 비교시 주소값을 비교하기때문에
	arr3 = arr2;
	...
	System.out.println(arr2[0]);
		...
		//내가 각 인덱스의 초기화를 하지 않아도 값들이 담겨있다.
				...
				// => 공간이 만들어 질 때 JVM이 기본값으로 라도 초기화를 진행한다.
		
		/*
		 * 실제 리터럴값을 곧바로 담을 수 있는 변수를 일반 변수라고 한다
		 * 주소값을 담고 있는 변수는 참조변수 라고 표현
		 * 
		 * 기본자료형(원시타입) : blooean, int, char byte, long, double 등등으로 선언된변수
		 * =>실제 리터럴값을 바로 담을 수 있다
		 * 
