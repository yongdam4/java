...
public class Run {
	...
	/*
	 * 1. 객체지향 언어 : "객체를 "지향"하는 언어로 "객체중심"으로 돌아간다
	 * 
	 * 2. 객체란 : 현실 세계에서 독립적(목적이 있고 가치가 있는 것)으로 존재하는 모든 것을 의미
	 ...
	 * 
		 ...
		 * 
		 * ex) 학생 관리 프로그램
		 * 1) 학생관련 객체(//훈련생, //훈련생, ///훈련생 등등...)
		 * 2) 공통적인 속성 및 기능(이름 , 나이, 주소, 전화번호, 키, 몸무게, 머리색, 발사이즈, 달리기 속도, 먹는다, 잔다, 화장실간다,공부한다, 휴식한다 등등)
	 ...
	 *  		김용담이라는 훈련생 객체 하나 만들기 -> String name1 = "김용담"; int age = 25; double height = 174.0;
	 *  		조석현이라는 훈련생 객체 하나 만들기 -> String name1 = "김용담"; int age = 25; double height = 170.0;
	 *  2. 배열로 프로그래밍 한다면
	 *  	배열 : 하나의 자료형으로 여러개의 값들을 보관할 수 있음
	 *  	이름을 보관하는 배열 String[] name = {"김용담","조석현"...}
	 *  	나이를 보관한는 배열 int[] age = {20,19....}
	 *  
	 *  3. 그래서 나온 개념 -> 구조체
	 *  구조체 : 여러개의 자료형이 여러개의 값들을 보관할 수 있다.
	 ...
	 * 외부로부터의 "데이터접근을 막고"
	 * 대신에 "데이터를 간접적으로 처리(값을 변경하고나, 가져오는 것등)" 할 메소드들을 클래스 내부에 작성하여 관리하는것
